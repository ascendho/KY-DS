...
// 算法2.2　顺序表的取值(按位序查找)
// 初始条件：线性表L已存在，且1≤i≤SqListLength(L)。
...
// 算法2.3 顺序表的查找(按值查找)
// 初始条件：线性表L已存在。
// 操作结果：返回L中第1个值与e相同的元素在L中的位序。若这样的数据元素不存在，则返回值为0
...
// 算法2.4 顺序表的插入
// 初始条件：线性表L已存在，且1≤i≤SqListLength(L)+1。
...
// 算法2.5 顺序表的删除
// 初始条件：线性表L已存在且非空，且1≤i≤SqListLength(L)。
// 操作结果：删除L的第i个数据元素，L的长度减1，并用e返回其值
